...
Recommended use:
- Project files are installed under C:\Users\Kane\Documents\Projects\Cylinder_Quote_To_HTML.
- Put Cylinder_Quote_Web_Milestone_1.zip in that folder.
...
PROJECT_ROOT = Path(r"C:\\Users\\Kane\\Documents\\Projects\\Cylinder_Quote_To_HTML")
...
def main() -> None:
    ...
    if validate_project(project_dir):
        ...
        local_zip = install_root / ZIP_NAME
        zip_path = local_zip if local_zip.is_file() else find_zip(script_dir)
        if zip_path is None:
            searched = "\n".join(f"  - {p}" for p in candidate_zip_paths(script_dir))
            pause_on_error(
                f"Could not find {ZIP_NAME}.\n\n"
                f"Put the Milestone 1 ZIP in this project folder and run it again:\n  {install_root}\n\n"
                f"Locations checked:\n{searched}"
            )
