...
function openAttachmentDialogFor(id) {
  const dialog = $('emailAttachmentDialog');
  const checkbox = $(id);
...
const INCLUDES_NUMBER_OPTIONS = [
  ...
  ['standard_rod_boot_qty', 'Rod Boot'],
  ['stop_tube', 'Stop Tube'],
  ['air_bleed_qty', 'Air Bleed'],
  ['extra_port_qty', 'Extra/Boss Ports'],
  ['extra_thread', 'Thread Extension'],
  ['extra_tie_rod_qty', 'Extended Tie Rod'],
  ['prox_switch_qty', 'Prox Switch'],
  ['legacy_post_discount_add', 'Add Money']
];

const INCLUDES_YES_NO_OPTIONS = [
  ['stainless_rod', 'Stainless Rod'],
  ['chrome_bore', 'Chrome Bore'],
  ['rod_gland_drain', 'Rod Gland Drain'],
  ['brass_wiper', 'Brass Wiper'],
  ['ultraox', 'UltraOx'],
  ['transducer', 'Transducer'],
  ['prepped_for_transducer', 'Prepped for Transducer'],
  ['sensor_cover', 'Sensor Cover'],
...
function renderQuotePreview(quote) {
  ...
  const dims = (quote.price_breakdown_snapshot || {}).dimensions
    || (quote.cylinder_inputs_snapshot || {}).dimensions
    || {};
  const dimValue = key => {
    const v = dims[key];
    if (v == null || v === '') return '—';
...
async function holdQuote() {
  if (!pendingQuotePayload) return;

  const button = $('holdQuoteButton');
  if (!button) return;
  $('previewErrorMessage').textContent = '';
  button.disabled = true;
  ...
  try {
    ...
    const holdResponse = await fetch(`/api/quotes/${saveBody.quote.id}/order/hold`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({})
    });
    ...
    $('previewErrorMessage').textContent = e.message || 'Hold failed';
    button.disabled = false;
...
function wireQuoteFormEvents() {
  ...
  if (holdQuoteButton) {
    holdQuoteButton.addEventListener('click', async () => {
      if (await holdQuote()) window.location.assign('/employee/dashboard');
    });
  }
  ...
  const attachQuoteFormButton = $('attachQuoteFormButton');
  if (attachQuoteFormButton) attachQuoteFormButton.addEventListener('click', () => openAttachmentDialogFor('attachQuoteForm'));
  const attachReportImagesButton = $('attachReportImagesButton');
  if (attachReportImagesButton) attachReportImagesButton.addEventListener('click', () => openAttachmentDialogFor('attachReportImage'));
  ...
  if (employeeDashboardButton) {
    employeeDashboardButton.addEventListener('click', async event => {
      if (!pendingQuotePayload || !currentQuote) return;
...
async function loadExistingQuote(quoteId) {
  ...
  pendingQuotePayload = {
    ...
    person_of_contact: q.person_of_contact || '',
...
async function loadDraftQuote() {
  ...
  const storedDraft = JSON.parse(draftJson);
  const payload = storedDraft.payload || storedDraft;
  pendingQuotePayload = payload;
  if (storedDraft.draft) {
    renderQuotePreview(buildQuoteLikeFromDraft(storedDraft.draft, payload));
    return;
  }
