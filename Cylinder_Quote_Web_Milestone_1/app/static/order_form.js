(() => {
  ...
  function buildDefaults(quote) {
    ...
    return {
      quote_number: quote.quote_number || '', order_number: '', order_date: localDate(quote.created_at), terms: 'NET 30',
      ...
      po_number: quote.reference_notes || '', tag: quote.customer_name || '', ship_method: 'UPS Ground - Collect',
      ship_date: new Date().toLocaleDateString('en-US'), contact: quote.customer_contact || '',
      ...
      parts: [...manualParts, ...specialParts],
      testing: [
  ...
  function render(quote) {
    ...
    while (partsHost.children.length < 24) addPartRow();
    testingHost.innerHTML = '';
    (data.testing || []).forEach(addTestingRow);
    while (testingHost.children.length < 4) addTestingRow();
