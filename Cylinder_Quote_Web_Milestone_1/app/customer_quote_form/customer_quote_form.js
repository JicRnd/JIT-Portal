/* Customer Quote Form page logic. */
const $ = id => document.getElementById(id);
const PORTAL_MODE = 'customer';
...
function selectedIncludes(inputs, manualItems) {
  ...
  const addItem = (label, quantity) => {
    ...
    if (Number.isFinite(numericQuantity) && numericQuantity > 0) {
      items.push({ label, quantity: numericQuantity });
    }
  };
...
function renderQuotePreview(quote) {
  ...
  $('pv_status').textContent = customerStatusLabel(quote.status);
  $('saveQuoteButton').textContent = isEditingExistingQuote ? 'Update Order' : 'Order Now';
  const createdByFooter = $('pv_created_by_footer');
  if (createdByFooter) createdByFooter.textContent = CURRENT_USER_NAME || quote.customer_name || '—';
...
async function saveQuote() {
  ...
  try {
    ...
    const existingQuoteId = currentQuote && currentQuote.id;
...
async function cancelOrder() {
  ...
  const button = $('cancelOrderButton');
  ...
  try {
    ...
    const body = await res.json();
    if (!res.ok || !body.ok) throw new Error(body.error || 'Order cancellation failed');
    window.location.assign('/customer/dashboard');
  } catch (e) {
    if (errorEl) errorEl.textContent = e.message || 'Order cancellation failed';
    if (button) {
...
function wireQuoteFormEvents() {
  ...
  if (backToOrderEntryButton) {
    backToOrderEntryButton.addEventListener('click', () => {
...
async function loadExistingQuote(quoteId, quoteNumber) {
  const loadUrl = quoteId
    ? '/api/quotes/' + encodeURIComponent(quoteId)
    : '/api/quotes/by-number/' + encodeURIComponent(quoteNumber);
...
async function init() {
  ...
  try {
    ...
    if (quoteId || quoteNumber) {
      isEditingExistingQuote = params.get('accepted') !== '1' && params.get('new') !== '1';
      await loadExistingQuote(quoteId, quoteNumber);
